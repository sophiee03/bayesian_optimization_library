...
class  OptimizationConfig:
    ...
    Attributes:
        objective_metrics (List): metrics to maximize/minimize
        optimization_parameters (List): parameters to optimize
        goal (List[str]): direction (MAX/MIN) of each metric
        ground_truth_dim (int): dim of the training dataset
    ...
    def details(self):
        """Method to print a summary of the configuration choices"""
        print(f"{'-'*60}")
        print(f"CONFIGURATION DETAILS:")
        print(f"    executing a {'SINGLE' if len(self.objective_metrics)==1 else 'MULTI'} objective model with:")
        print(f"    - Parameters to optimize: {self.optimization_parameters}")
        print(f"    - Metrics to maximize/minimize: {self.objective_metrics}")
        print(f"    The directions of the metrics are: {self.goal}")
    ...
    def return_dict(self):
        """Method to generate a dictionary of the configuration attributes"""
        return {
...
class BoundsGenerator():
    """Class to create parameter bounds needed for normalization
    ...
    def generate_bounds(self, t: torch.Tensor):
        """Method to generate the bounds based on the min and max value of the parameter
    ...
    def generate_norm_bounds(self, n: int):
        """Method to generate bounds normalized [[0.0][1.0]]*n
...
class Timer:
    ...
    def _record_timings(self, name: str, elapsed: float):
        """Method to register timings measured
    ...
    def time_function(self, name: str = None):
        """Method to register the time that a function takes to execute
    ...
    def get_opt_time(self, name: str) -> float:
        """Method to obtain only the measured time required
        
        Args:
            name (str): saved name
        """
        if name not in self.timings.keys():
            raise ValueError(f"Time not found for {name}")
        return self.timings[name]

    def reset(self):
        """Method to clear all times registered"""
        self.timings.clear()